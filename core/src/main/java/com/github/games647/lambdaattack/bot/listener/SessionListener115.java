...
import com.github.fgruslan.lambdaattack.Utils;
...
public class SessionListener115 extends SessionListener {
	...
	private boolean didCaptcha = false;
	...
	public void packetReceived(PacketReceivedEvent receiveEvent) {
		if (receiveEvent.getPacket() instanceof ServerChatPacket) {
			...
			/**Captcha catcher**/
			if(message.getFullText().contains("каптчу") && !didCaptcha) {
				Utils.currentCaptcha = message.getFullText();
				owner.sendMessage(Utils.extract());
				didCaptcha = true;
				try {
